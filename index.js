...
app.post('/buy_card', (req, res) => {
  ...
  const validatePlayerEnoughToBuyCard = (card) => {
    return Object.keys(card.price).reduce((newPlayer, color) => {
      if(!newPlayer) return newPlayer;
      const cardWithColor = newPlayer.cards.filter(x => x.property === color);
      if(card.price[COLOR] <= cardWithColor.length) return newPlayer;
      
      const tokenWithColor = newPlayer.token[color];
      if(card.price[COLOR] <= cardWithColor.length + tokenWithColor) return newPlayer;

      const tokenGold = newPlayer.token[COLOR.GOLD];
      if(card.price[COLOR] <= cardWithColor.length + tokenWithColor + tokenGold){
        newPlayer.token[COLOR.GOLD] -= (card.price[COLOR] - cardWithColor.length - tokenWithColor);
        return newPlayer;
      }
      return newPlayer;
    }, cloneDeep(player));
  ...
  const removeTokenFromBuying = (tokenToBuy, card) => {
    return Object.keys(card.price).reduce((newToken, color) => {
      ...
      const tokenWithColor = tokenToBuy.token[color];
      if(!tokenWithColor) return undefined;
      if(card.price[COLOR] <= cardWithColor.length + tokenWithColor) {
        newToken[COLOR] -= (card.price[COLOR] - cardWithColor.length);
        return newToken;
      }

      const tokenGold = tokenToBuy.token[COLOR.GOLD];
  ...
  if(card){
    ...
    if(!validatePlayerEnoughToBuyCard(card)) return sendBadRequest(res);
    const newToken = removeTokenFromBuying(token, card);
    if(!newToken) return sendBadRequest(res);
